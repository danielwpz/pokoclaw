...
**Delegated approval.** When a TaskAgent or cron job needs approval, the system does not surface a card directly to the user and wait. Instead, the Main Agent evaluates the request in a dedicated approval session using a restricted tool set — read-only investigation tools plus a dedicated `review_permission_request` tool. This session is for evaluation only; it does not continue executing the task itself.
