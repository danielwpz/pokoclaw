...
**Managed shell processes.** Ordinary bash calls remain synchronous for compatibility. Main and SubAgents can explicitly use `yieldMs` for a slow finite command or `background: true` for an intentional service. Pokoclaw owns the resulting process tree, persists its status and bounded output tail, applies a total-lifetime timeout, and exposes `process` actions for list, poll/log, and kill. If a handed-off process later exits, yielded work defaults to proactively waking the Agent while intentional background services default to adding hidden context to the next user-driven turn; the Agent can override either default when the user's intent requires it. Native shell detachment such as `&`, `nohup`, `START /B`, or `Start-Job` remains blocked because it would escape this lifecycle. Managed processes are terminated during graceful runtime shutdown; records that were active across an unclean restart are marked `lost`.
...
Main Agent and SubAgent chats can use `/clear` when repeated compaction has made the effective context noisy. The user-visible session and full SQLite transcript stay intact, while the runtime replaces only the LLM-visible context:
