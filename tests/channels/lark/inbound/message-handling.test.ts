...
describe("lark inbound message handling", () => {
  ...
  test("routes think tank thread follow-up into the same consultation instead of runtime ingress", async () => {
    await withHandle(async (handle) => {
      seedFixture(handle);
      handle.storage.sqlite.exec(`
        INSERT INTO sessions (id, conversation_id, branch_id, owner_agent_id, purpose, status, created_at, updated_at)
        ...
        INSERT INTO think_tank_consultations (
          ...
          'How should think tank threads behave?', 'Need thread follow-up to continue the same consultation.',
          '2026-03-27T00:00:03.000Z', '2026-03-27T00:00:04.000Z'
        );
        ...
        INSERT INTO channel_threads (
          ...
          'thread_tt_1', 'lark', 'default', 'conv_main', 'oc_chat_1', 'omt_tt_thread_1',
      ...
      await handler({
        ...
        message: {
          message_id: "om_tt_thread_msg_1",
          parent_id: "om_user_reply_tt",
          thread_id: "omt_tt_thread_1",
          chat_id: "oc_chat_1",
          chat_type: "p2p",
          message_type: "text",
          create_time: "1774569600000",
          content: JSON.stringify({
            text: "继续追问：如果一个 consultation 正在运行，thread 新消息该怎么处理？",
          }),
        },
      });
      ...
      expect(thinkTanks.continueConsultation).toHaveBeenCalledExactlyOnceWith({
        consultationId: "tt_1",
        prompt: "继续追问：如果一个 consultation 正在运行，thread 新消息该怎么处理？",
        createdAt: new Date("2026-03-27T00:00:00.000Z"),
      });
