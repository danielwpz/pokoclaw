...
describe("agent loop bash approval flow", () => {
  ...
  test("pauses and resumes the same bash tool call after approval", async () => {
    ...
    executeSandboxedBashMock
      .mockRejectedValueOnce(
        toolApprovalRequired({
          request: {
            scopes: [{ kind: "fs.write", path: "/tmp/notes.txt" }],
          },
          reasonText: "This tool needs approval to continue: Write /tmp/notes.txt.",
        }),
      )
    ...
    expect(executeSandboxedBashMock).toHaveBeenCalledTimes(2);
