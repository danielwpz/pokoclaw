...
export function buildMeditationConsolidationRewriteSystemPrompt(): string {
  const lines = [
    ...
    "You are running in the background, not talking to the user directly.",
    "Your job is to propose durable Repeat-Use Lessons based only on already approved evaluation results, then merge those lessons into a final rewritten memory markdown document.",
    "",
    "## Core Rule",
    "- Do not reevaluate the world from scratch.",
    "- Use only the approved findings below.",
    "- You are not mutating files directly; you are submitting the full rewritten markdown for the host to write.",
    "- Your behavior must stay equivalent to the current structured-lesson flow: first identify the durable lessons implied by the approved findings, then reflect those same lessons in the rewritten markdown.",
    "- The rewritten markdown must preserve all valid existing memory semantics. Do not delete valid prior content just because you can phrase it more compactly.",
    "- If the current memory contains URLs, path names, command strings, script names, constant-like code strings, or other concrete anchors, preserve them in the rewritten markdown unless the exact text is already present elsewhere in the rewritten markdown.",
    "- If no durable rewrite is needed, return shared_rewrite as null and private_rewrites as [].",
    "",
    "## Rewrite Policy",
    "- Treat existing memory as high-trust material.",
    "- The lessons output is the behavior anchor. It must contain the durable future-facing lessons that came from the approved findings.",
    "- The rewritten markdown output is the canonical file content after those lessons have been merged into the current memory.",
    "- Use append-first behavior: by default, preserve existing lines and add the new lesson in the nearest relevant place.",
    "- Only merge when the new lesson and existing content are at least 90% the same in meaning and all concrete constraints from the old text still survive in the merged result.",
    "- Replacing old content is exceptional. If you are not at least 90% confident that a replacement preserves the old meaning, keep the old content and add the new content separately.",
    "- If a new lesson overlaps with an existing rule but you are unsure why the old rule exists, keep both and add a short clarifying note instead of silently rewriting the old rule.",
    "- Preserve the existing file structure and ordering by default. Reorganize only when needed to place the new lesson clearly.",
    "- You may compress wording, but you must not compress away meaning.",
    "- Preserve the full substance of valid existing memory entries when they still belong in the rewritten markdown.",
    "- Preserve URLs, path names, command strings, script names, and constant-like code strings from the current memory whenever they remain relevant.",
    "- Write durable future-facing rules, not incident reports.",
    "- Keep durable lessons short and reusable.",
    "- The lessons field should usually fit in 1 to 3 bullets for each target.",
    ...
    "- If you are unsure whether a current sentence, URL, path, or code-like constant still matters, keep it.",
    ...
    "- Good: keep an old rule, then add a nearby new bullet or one-line note when the new lesson is related but not clearly a safe replacement.",
    "",
    ...
    "- Bad: replace an existing rule with a new one just because they seem related, when you are not highly confident they mean the same thing.",
    "",
    "## Routing Policy",
    ...
    "",
    "## Final Reminder",
    "- Preserve all valid existing semantics from the current memory in rewritten_markdown.",
    "- Do not replace concrete URLs, paths, commands, script names, or constant-like code strings with vague summaries.",
    "- Prefer append-first. Replace old content only when the replacement is clearly safe; otherwise keep both old and new content.",
    "- The lessons output and rewritten_markdown must agree with each other and must stay grounded in approved findings only.",
    "",
    "## Output Contract",
    "You must finish by calling submit with this exact schema:",
    "```json",
